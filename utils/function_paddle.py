...
from utils.utils_paddle import AverageMeter
from utils.utils_paddle import get_confusion_matrix
from utils.utils_paddle import adjust_learning_rate
...
def train(config, epoch, num_epoch, epoch_iters, base_lr,
    ...
    for i_iter, batch in enumerate(trainloader, 0):
        ...
        labels = labels.astype('int').cuda()
        bd_gts = bd_gts.astype('float').cuda()
        ...
        if i_iter % config.PRINT_FREQ == 0:
            msg = 'Epoch: [{}/{}] Iter:[{}/{}], Time: {:.2f}, ' \
                  'lr: {}, Loss: {:.6f}, Acc:{:.6f}, Semantic loss: {:.6f}, BCE loss: {:.6f}, SB loss: {:.6f}' .format(
                      ...
                      batch_time.average(), optimizer.get_lr(), ave_loss.average(),
...
def testval(config, test_dataset, testloader, model,
    ...
    with paddle.no_grad():
        for index, batch in enumerate(tqdm(testloader)):
            ...
            if pred.shape[-2] != size[-2] or pred.shape[-1] != size[-1]:
                pred = F.interpolate(
                    pred, size[-2:],
                    mode='bilinear', align_corners=config.MODEL.ALIGN_CORNERS
                )
