...
if __name__ == '__main__':
    ...
    model = models.pidnet_paddle.get_pred_model(args.a, 19 if args.c else 11)
    ...
    with paddle.no_grad():
        for img_path in images_list:
            ...
            img = paddle.to_tensor(img).unsqueeze(0).cuda()
            pred = model(img)
            pred = F.interpolate(pred, size=img.shape[-2:], 
                                 mode='bilinear', align_corners=True)
            pred = paddle.argmax(pred, axis=1).squeeze(0).cpu().numpy()
            ...
            if not os.path.exists(sv_path):
                os.mkdir(sv_path)
            sv_img.save("samples/outputs/"+img_name)
